 ...
 */
 ...
 /*
   ...
   All error messages send to stderr instead of stdout.
	...
	if (device_no < 0) {
		...
		if (VXI11_DEVICE_NO >= VXI11_MAX_CLIENTS) {
			// printf("Error: maximum of %d clients allowed\n",VXI11_MAX_CLIENTS);
                        fprintf(stderr, "Error: maximum of %d clients allowed\n",VXI11_MAX_CLIENTS); // RN 2010-12-13
...
int     device_no = -1;
	...
	if (device_no == -1) {
		...
		//printf("                    with IP address %s\n",ip);
                fprintf(stderr, "vxi11_close_device: error: I have no record of you ever opening device\n"); // RN 2010-12-13
                fprintf(stderr, "                    with IP address %s\n",ip);
		ret = -4;
		}
...
char		scan_cmd[20];
	...
	if (in_buffer[0] != '#') {
		...
		//printf("First 20 characters received were: '");
                fprintf(stderr, "vxi11_user: data block error: data block does not begin with '#'\n");
                fprintf(stderr, "First 20 characters received were: '"); // RN 2010-12-13     
		for(l=0;l<20;l++) {
			//printf("%c",in_buffer[l]);
                        fprintf(stderr, "%c",in_buffer[l]); // RN 2010-12-13  
			}
		// printf("'\n");
                fprintf(stderr, "'\n"); // RN 2010-12-13  
		return -3;
		...
		if (ret != 0) {
			if (ret != -VXI11_NULL_WRITE_RESP) {
				...
				//printf("       The function vxi11_send returned %d. ",ret);
                                fprintf(stderr, "Error: vxi11_send_and_receive: could not send cmd.\n"); // RN 2010-12-13
                                fprintf(stderr, "       The function vxi11_send returned %d. ",ret);
				return -1;
				}
			else // printf("(Info: VXI11_NULL_WRITE_RESP in vxi11_send_and_receive, resending query)\n");
                             fprintf(stderr, "(Info: VXI11_NULL_WRITE_RESP in vxi11_send_and_receive, resending query)\n"); // RN 2010-12-13
			}
		...
		if (bytes_returned <= 0) {
			if (bytes_returned >-VXI11_NULL_READ_RESP) {
				...
				//printf("       The function vxi11_receive returned %ld. ",bytes_returned);
                                fprintf(stderr, "Error: vxi11_send_and_receive: problem reading reply.\n");
                                fprintf(stderr, "       The function vxi11_receive returned %ld. ",bytes_returned); // RN 2010-12-13
				return -2;
				}
			else // printf("(Info: VXI11_NULL_READ_RESP in vxi11_send_and_receive, resending query)\n");
                        fprintf(stderr, "(Info: VXI11_NULL_READ_RESP in vxi11_send_and_receive, resending query)\n"); // RN 2010-12-13
			}
	...
	do {
		...
		if (write_resp.error != 0) {
			//printf("vxi11_user: write error: %d\n", (int)write_resp.error);
                        fprintf(stderr, "vxi11_user: write error: %d\n", (int)write_resp.error);// RN 2010-12-13
	...
	do {
 		...
 		if (read_resp.error != 0) {
			...
			*/
			...
			// printf("vxi11_user: read error: %d\n", (int)read_resp.error);
                        fprintf(stderr, "vxi11_user: read error: %d\n", (int)read_resp.error); // RN 2010-12-13
		...
		if( (read_resp.reason & RCV_END_BIT) || (read_resp.reason & RCV_CHR_BIT) ) {
			...
			//printf("xvi11_user: read error: buffer too small. Read %d bytes without hitting terminator.\n", (int)curr_pos );
                        fprintf(stderr, "xvi11_user: read error: buffer too small. Read %d bytes without hitting terminator.\n", (int)curr_pos ); // RN 2010-12-13
			return -100;
			}
		} while(1);
